...
где – значение для аргумента sigmaX; – значение для аргумента sigmaY,
ksizew – ширина фильтра; ksizeh – высота фильтра. Таким образом, параметры вычисляются по размеру окна. Графическое представление фильтра приведено на рис. 1.7.

![ Графическое представление фильтра. ](https://raw.githubusercontent.com/kzvyagin/introduction_to_cv_and_ml/main/chapter2/images/gauss_core_1.6.png)

Графическое представление фильтра размером 17х17, с параметром нормального распределения зависящим от размера фильтра.


<h3>Практическое занятие №...</h3>

Также помимо свертки с указанными фильтрами OpenCV предоставляет возможность использовать свертку с произвольным линейным фильтром. Чтобы создать фильтр, нужно создать двумерный массив нужного размера, каждый элемент которого будет числом с плавающей точкой.

Чтобы фильтр был нормированным, необходимо будет вручную разделить значения на сумму элементов фильтра. Применить фильтр можно с помощью функции filter2D().

```
img_new = cv2.filter2D(
	img,# входное изображение
	ddepth, # глубина изображения
	kernel, # сгенерированное ядро
	anchor, # положение якоря
	delta, # добавляемое значение
	bType # тип границы
)
```

Параметры img, ddepth, anchor и bType принимают те же значения, что и в случае с другими фильтрами. В аргумент kernel передается переменная, которая хранит ядро. Параметр delta задавать не обязательно. В случае если для данного параметра указано значение, оно будет добавлено к каждому пикселю результирующего изображения после применения фильтра. Возвращаемое значение функции – новое изображение.

<h3>Пороговые преобразования.</h3>

Пороговые преобразования универсальный механизм применяемый во многих сферах информационных технологий и математики. Конкретно в обработке изображений может быть использован для:

    •  выделения главной цветовой составляющей
    •  бинаризации
    •  фильтрации контуров 
    •  уменьшения шума изображения

С точки зрения математики для каждого пикселя (x, y) изображения I выполнялось следующее преобразование, которое можно описать формулой 2.2:

![ формулой 2.2 ](https://raw.githubusercontent.com/kzvyagin/introduction_to_cv_and_ml/main/chapter2/images/formula_2_2.png)

где I1(x, y) – пиксель на исходном изображении; I2(x, y) – пиксель на изображении, содержащем результат; T – порог яркости; действие 1 и действие 2 –условные обозначения двух вариантов изменения значения яркости пикселя. Такое преобразование называется пороговым.
